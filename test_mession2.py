...
class MissionTransfer:
    ...
    def connect_drones(self):
        ...
        self.server_conn = mavutil.mavlink_connection('udp:127.0.0.1:14541',source_system=127)
        self.server_conn.wait_heartbeat()
        
        print("Connecting to client drone...")
        self.client_conn = mavutil.mavlink_connection('udp:127.0.0.1:14542',source_system=127)
    ...
    def upload_to_client(self, mission_items, start_seq):
        ...
        for seq, item in enumerate(mission_items):
            # Wait for request
            while True:
                req = self.client_conn.recv_match(type='MISSION_REQUEST',
                                                blocking=True,
                                                timeout=self.timeout)
                print(req)
